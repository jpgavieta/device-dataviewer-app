...
def build_phone_df(df):
    ...
    if "Phone GPS" in df.columns:
        phone_df['gps_phone_bool'] = df['Phone GPS'].map({'yes': 1, 'no': 0}) # Was the phone GPS currently used for this data point? (yes/no --> 1/0)
    ...
    phone_df = phone_df.replace({'yes': 1, 'no': 0}) # For any remaining yes/no columns (e.g. motion), convert to bools (1/0)
    ...
    phone_df["charge_phone_bool"]   = phone_df["charg_phone_raw"].map({"no": 0, "cd": 0, "yes": 1})   # Was the phone currently charging during data collection? (yes/no --> 1/0)
    phone_df["cooldown_phone_bool"] = phone_df["charg_phone_raw"].map({"no": 0, "cd": 1, "yes": 0})   # Was the phone currently in cooldown AFTER charging (yes/no --> 1/0, cd (cooldown) --> 1, treated as a separate boolean since cooldown may impact phone's performance)
